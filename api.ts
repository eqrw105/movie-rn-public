...
export const movieApi = { 
    ...
    upcoming:({pageParam}) => fetch(`${BASE_URL}/movie/upcoming?api_key=${API_KEY}&language=en-US&page=${pageParam}`).then(res => res.json()),
...
export const tvApi = { 
    ...
    search:({queryKey}) => {
        const [_, query] = queryKey;
        return fetch(`${BASE_URL}/search/tv?api_key=${API_KEY}&query=${query}`).then(res => res.json());
    },
    detail:({queryKey}) => {
        const [_, id] = queryKey;
