...
const Detail: React.FC<DetailScreenProps> = ({navigation: { setOptions }, route: { params }}) => {
    ...
    const { isLoading, data } = useQuery([isMovie ? "movies" : "tv", params.id], isMovie ? movieApi.detail : tvApi.detail);
    ...
    <Container>
        ...
        <Data>
        {data?.videos?.results?.map(video => <VideoBtn key={video.key} onPress={() => openYTLink(video.key)}><Ionicons name="logo-youtube" color="white" size={24} /><BtnText>{video.name}</BtnText></VideoBtn>)}
        </Data>
    </Container>
    );
};
